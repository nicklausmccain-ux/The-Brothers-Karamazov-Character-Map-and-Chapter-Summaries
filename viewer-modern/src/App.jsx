...
const BOOK = "brothers-karamazov";
// import.meta.env.BASE_URL includes trailing slash (e.g. "/novel-character-relationship-map/")
// We build BASE so it never has a double-slash and works on both local dev and GitHub Pages.
const BASE = `${import.meta.env.BASE_URL}books/${BOOK}`.replace(/\/\//g, "/");
...
export default function App() {
  ...
  const [mobileView, setMobileView] = useState("graph"); // "graph" | "summary" | "detail"

  const cyRef = useRef(null);
  const isMobile = useIsMobile(900);

  // ── Go to Chapter 1 (brand click) ──
  ...
  // ── On mount: try chapters first, fall back to whole-book ──
  useEffect(() => {
    (async () => {
      try {
        ...
        setChapterNum(chapIdx[0]?.chapter ?? 1);
        setMode("chapters");
      } catch (_chapErr) {
        try {
  ...
  }, [mode, chapters, chapterNum]);

  // ── Compute core/total stats from raw snapshot ──
  const stats = useMemo(() => {
  ...
  // ── Build node lookup for importance ──
  ...
  // ── Build Cytoscape elements with Focus + importance ──
  const elements = useMemo(() => {
    ...
    // If focus mode + a specific node selected, narrow to neighborhood
  ...
  // Re-attach events when dependencies change
  useEffect(() => {
  ...
  // ── Layout re-run with graphReady gate ──
  useEffect(() => {
  ...
  // ── Reset View (bulletproof state reset) ──
  ...
  // ── Story summary (narrative from LLM) ──
  ...
  // Graph analytics summary (fallback)
    ...
    return (
      <div className="loading-screen">
        <div className="loading-content">
          <h1 className="loading-title">The Brothers Karamazov</h1>
          <p className="loading-subtitle">
            Loading the world of Dostoevsky...
          </p>
  ...
  // ── Left sidebar content (reused in desktop and mobile) ──
  const leftPanelContent = (
    <>
                ...
                {keyCharacters.length > 0 && (
                  <div className="key-characters-row">
                    {keyCharacters.map((name, i) => (
                      <span key={i} className="key-char-badge">
                        {name}
                      </span>
                    ))}
                  </div>
      ...
      {/* New in this chapter — clickable character + relationship list */}
              ...
              {delta.new_nodes.slice(0, 8).map((nid) => (
                ...
                >
                  ...
                  <span className="new-item-name">
                    {characters[nid]?.name ||
                      titleCase(nid.replace(/-/g, " "))}
                  </span>
                </button>
              ))}
              {delta.new_nodes.length > 8 && (
                <div className="new-more">
                  and {delta.new_nodes.length - 8} more&hellip;
                </div>
              )}
            </div>
          )}
          {delta.new_edges.length > 0 && (
            <div className="new-list">
              ...
              {delta.new_edges.slice(0, 8).map((e, i) => {
                const srcName =
                  characters[e.source]?.name ||
                  titleCase(e.source.replace(/-/g, " "));
                const tgtName =
                  characters[e.target]?.name ||
                  titleCase(e.target.replace(/-/g, " "));
                ...
                return (
                  ...
                  >
                    ...
                    <span className="new-item-name">
                      ...
                      {e.label && (
                        <span className="new-item-label"> ({e.label})</span>
                      )}
                    </span>
                  </button>
                );
              ...
              {delta.new_edges.length > 8 && (
                <div className="new-more">
                  and {delta.new_edges.length - 8} more&hellip;
                </div>
              )}
            </div>
          )}
        ...
        <div className="chapter-progress-section">
          ...
          <div className="progress-bar-container">
            <div
              ...
              style={{
                width: `${(chapterNum / chapters.length) * 100}%`,
              }}
            />
          </div>
      ...
      <div className="options-section">
        ...
        <label className="option-row">
          ...
          <span>
            Core cast only
            <span className="option-hint"> (hides minor characters)</span>
          </span>
        </label>
        <label className="option-row">
          <input
  ...
  // ── Right sidebar content (reused in desktop and mobile) ──
    ...
    <>
      {!selected && (
        <div className="detail-empty">
          ...
          <p>
            Click any character or relationship in the graph to see details.
          </p>
          {focusMode && (
            <p className="focus-hint">
              Focus mode active. Click a node to isolate its neighborhood.
            </p>
          )}
        </div>
      )}
        ...
        <div className="detail-panel">
          ...
          {focusMode && (
            <button
              ...
              onClick={() => setFocusNodeId(selected.id)}
            >
          ...
          <div className="section-title">
            Connections ({selected.neighbors.length})
          </div>
          <div className="connection-list">
            {selected.neighbors.map((n, i) => (
              <div
                ...
                title={n.description || `${n.name} - ${n.relation}`}
              >
      ...
      {selected?.type === "edge" && (
        <div className="detail-panel">
          ...
          <p className="detail-description">{selected.description}</p>
          {selected.isNew && (
            <span className="badge new">New this chapter</span>
          )}
          <div className="edge-actions">
            <button
              className="edge-jump-btn"
              onClick={() => jumpToNode(selected.sourceId)}
            >
              Go to {selected.sourceName.split(" ")[0]}
            </button>
            <button
              className="edge-jump-btn"
              onClick={() => jumpToNode(selected.targetId)}
            >
              Go to {selected.targetName.split(" ")[0]}
            </button>
          </div>
        </div>
      )}
  ...
  const graphContent = (
    <>
      <div className="graph-noise" />
      <div className="graph-glow" />
      {elements.length > 0 ? (
        <div className={`graph-wrap ${graphReady ? "ready" : "booting"}`}>
          {layouting && (
            <div className="graph-overlay">Laying out&hellip;</div>
          )}
          <CytoscapeComponent
            elements={elements}
        ...
        <div className="graph-empty">
          <p>No data to display for this chapter.</p>
        </div>
      )}
    </>
  );
  ...
  return (
    <div className={`app-shell ${isMobile ? "mobile" : ""}`}>
      ...
      <header className="topbar">
        ...
        >
          <span
            className="brand-portrait-wrap"
            data-tooltip="Fyodor Dostoevsky · 1880"
          >
           <img
  src={`${import.meta.env.BASE_URL}dostoevsky.jpg`}
  alt="Dostoevsky portrait"
  className="brand-img"
/>

          </span>
          <span className="brand">The Brothers Karamazov</span>
        </button>
        ...
        <div className="topbar-center">
          {mode === "chapters" && (
            <div className="nav-controls">
              ...
              <button
                className="nav-btn"
                onClick={goNext}
                disabled={chapterNum >= chapters.length}
                title="Next chapter"
          ...
          {mode === "whole-book" && (
            <span className="mode-label">
              Whole Book View (chapter data not yet available)
            </span>
          )}
        </div>
        ...
        <div className="topbar-right">
          <button
            ...
            onClick={resetView}
            title="Reset selection, search, focus, and fit to view"
          ...
          <button
            ...
            aria-label="Re-run graph layout"
          >
            Re-layout
          </button>
          <button
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!nextFocus) setFocusNodeId(null);
            }}
            title={
              focusMode
                ? "Exit focus mode"
                : "Focus: show core cast only, click node to isolate neighborhood"
            }
      ...
      {isMobile && (
        <nav className="mobile-tabs" role="tablist">
          <button
            className={`mobile-tab ${mobileView === "graph" ? "active" : ""}`}
          ...
          <button
            className={`mobile-tab ${mobileView === "summary" ? "active" : ""}`}
      ...
      {isMobile && (
        <>
          {mobileView === "graph" && (
            <main className="graph-area mobile-panel">{graphContent}</main>
          )}
          {mobileView === "summary" && (
            <aside className="sidebar-left mobile-panel">
              {leftPanelContent}
            </aside>
          )}
          {mobileView === "detail" && (
            <aside className="sidebar-right mobile-panel">
              {rightPanelContent}
            </aside>
          )}
        </>
